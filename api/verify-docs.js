...
Respond with JSON only, no other text.`
        ...
        if (!anthropicData.content) {
            return new Response(JSON.stringify({ 
                approved: false, 
                summary: "API error: " + JSON.stringify(anthropicData)
            }), {
                status: 200,
        ...
        try {
            ...
            return new Response(JSON.stringify({ approved: false, summary: "Could not parse verification result: " + resultText }), {
                status: 200,
                headers: { 'Content-Type': 'application/json' }
            });
        }
